...
class Landscape: Node, Texturable
{
    ...
    var terrain = Terrain(size: [8, 8], height: 1, maxTessellation: UInt32(Landscape.maxTessellation))
    ...
    init(name: String)
    {
        renderPipelineState = Landscape.buildRenderPipelineState()
        tessellationPipelineState = Landscape.buildComputePipelineState()
        
        do
        {
            heightMap = try Landscape.loadTexture(imageName: "mountain")
            cliffTexture = try Landscape.loadTexture(imageName: "cliff-color")
            snowTexture = try Landscape.loadTexture(imageName: "snow-color")
            grassTexture = try Landscape.loadTexture(imageName: "grass-color")
        }
        catch
        {
            fatalError(error.localizedDescription)
        }
        terrainSlope = Landscape.heightToSlope(source: heightMap!)
    ...
    static func buildRenderPipelineState() -> MTLRenderPipelineState
    {
        ...
        descriptor.maxTessellationFactor = Landscape.maxTessellation
